...
GraveFallGame.scene.Game.prototype.startPlayerPowerUpShake = function (playerMenu) {
    ...
    if (playerMenu.stand.visible !== true) {
        playerMenu.previewRestoreHidden = true;
        playerMenu.stand.visible = true;
        playerMenu.stand.alpha = 1;
    }

    playerMenu.previewShakeTimerMs = Math.max(playerMenu.previewShakeTimerMs || 0, 320);
    playerMenu.previewShakeDurationMs = Math.max(playerMenu.previewShakeDurationMs || 0, 320);
    playerMenu.previewShakeAmountX = 7;
...
GraveFallGame.scene.Game.prototype.spawnMinigamePowerUpEffect = function (menu) {
    ...
    if (!menu || !menu.minigame || menu.minigame.isFinalCharge !== true || !this.stage || typeof this.spawnParticleBurstAt !== "function") {
        return;
    }

    this.startPlayerPowerUpShake(menu);
    anchor = this.getBuffEffectAnchor(menu);
    colors = this.getBuffEffectColors(menu);

    this.spawnParticleBurstAt(anchor.x, anchor.y + 4, {
        effectType: "bossFinalChargePowerUp",
        colors: [colors.primary, colors.dark, colors.secondary || colors.primary, "#FFFFFF"],
        count: 12,
        durationMs: 560,
        minDistance: 4,
        maxDistance: 28,
...
GraveFallGame.scene.Game.prototype.completeButtonMashCycle = function (menu) {
    ...
    if (this.isBossPowerUpMinigameActive(menu)) {
        this.spawnMinigamePowerUpEffect(menu);
    }
